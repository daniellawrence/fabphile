...
ALLOWED_GIT_COMMAND = ['clone', 'fetch', 'pull', 'commit', 'push', 'update', 'describe']

def invoke_git(git_command, args=[]):
    command_arguments = ""
    check(git_command in ALLOWED_GIT_COMMAND, "%s command is not allowed" % git_command)
    if type(args) == type([]):
...
def fetch():
    ...
    check(not current_dir_is_gitrepo(), "You can only update an existing git-repo")
    invoke_git("fetch")
...
def archive(archive_name):
    ...
    check(current_dir_is_gitrepo(), "You can only archive an existing git-repo")
    invoke_git("archive", "--format=tar %s" % archive_name)

def version():
    "Return the current version"
    check(not current_dir_is_gitrepo(), "You can only descibe an existing git-repo")
    with settings(hide('everything')):
